...
//    uint8_t localhost_accept_ch_alps[52] = {
//	// Offset 0x00000000 to 0x00000033
//	0x00, 0x00, 0x2B, 0x89, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x68,
//	0x74, 0x74, 0x70, 0x73, 0x3A, 0x2F, 0x2F, 0x6C, 0x6F, 0x63, 0x61, 0x6C,
//	0x68, 0x6F, 0x73, 0x74, 0x00, 0x16, 0x73, 0x65, 0x63, 0x2D, 0x63, 0x68,
//	0x2D, 0x75, 0x61, 0x2D, 0x66, 0x75, 0x6C, 0x6C, 0x2D, 0x76, 0x65, 0x72,
//	0x73, 0x69, 0x6F, 0x6E
//    };
//localhost_origin
...
ngx_log_error(NGX_LOG_INFO, c->log, 0,
                             "44");
    uint8_t proto[2] = "h2";
   char origin_https_part[] = "https://";
//   char temp_servername[] = "ttt.local.vexstud.io";
   char accept_ch[] = "sec-ch-ua,ua,sec-ch-ua-platform,ua-platform,platform,sec-ch-ua-mobile,ua-mobile,sec-ch-ua-full-version,ua-full-version,sec-ch-ua-full-version-list,ua-full-version-list,sec-ch-ua-platform-version,ua-platform-version,sec-ch-ua-arch,ua-arch,arch,sec-ch-ua-bitness,ua-bitness,sec-ch-ua-model,ua-model,model,sec-ch-ua-wow64,ua-wow64,sec-ch-lang,lang,sec-ch-save-data,save-data,sec-ch-width,width,sec-ch-viewport-width,viewport-width,sec-ch-viewport-height,viewport-height,sec-ch-dpr,dpr,sec-ch-device-memory,device-memory,sec-ch-rtt,rtt,sec-ch-downlink,downlink,sec-ch-ect,ect,sec-ch-prefers-color-scheme,sec-ch-prefers-reduced-motion,sec-ch-prefers-reduced-transparency,sec-ch-prefers-contrast,sec-ch-forced-colors,sec-ch-prefers-reduced-data";
   uint8_t* ptr;
...
ngx_log_error(NGX_LOG_INFO, c->log, 0,
                             "44x");
//    u_char *full_origin;
   u_char *accept_ch_data;
   uint32_t accept_ch_size;
//    full_origin = ngx_pcalloc(c->pool, ngx_strlen(origin_https_part) + ngx_strlen(servername))
//    ngx_memcpy(full_origin, origin_https_part, ngx_strlen(origin_https_part));
//    ngx_memcpy(((u_char *)(&full_origin))+ngx_strlen(origin_https_part), servername, ngx_strlen(servername));
//    uint8_t* ptr;
//    uint32_t len_l, len_l_n;
//    uint16_t len, len_n;
...
//    accept_ch = value[2].data;
   ...
   len_l = ngx_strlen(origin_https_part) + ngx_strlen(servername) + ngx_strlen(accept_ch) + 4;
   accept_ch_data = ngx_pcalloc(c->pool, len_l+9);
//   accept_ch_data = ngx_alloc(len_l+9, c->log);
   accept_ch_size = len_l+9;
   ptr = accept_ch_data;
...
ngx_log_error(NGX_LOG_INFO, c->log, 0,
                             "44z");
//   ptr = accept_ch_data;
//   ngx_memcpy(ptr, cscf->accept_ch, 3+1+1+2+2);
   ngx_log_error(NGX_LOG_INFO, c->log, 0,
                                "44z1");
//   ptr+=3+1+1+2+2;
   len = ngx_strlen(origin_https_part) + ngx_strlen(servername);
   ngx_log_error(NGX_LOG_INFO, c->log, 0,
                                "44z2");
   len_n = htons(len);
   ngx_memcpy(ptr, (uint8_t *)(&len_n), 2);
   ptr += 2;
   ngx_memcpy(ptr, origin_https_part, ngx_strlen(origin_https_part));
   ngx_log_error(NGX_LOG_INFO, c->log, 0,
                                "44z3");
   ptr += ngx_strlen(origin_https_part);
   ngx_memcpy(ptr, servername, ngx_strlen(servername));
   ngx_log_error(NGX_LOG_INFO, c->log, 0,
                                "44z4");
   ptr += ngx_strlen(servername);
//    ptr += 2;
...
ngx_log_error(NGX_LOG_INFO, c->log, 0,
                             "44");
//   ngx_log_error(NGX_LOG_INFO, c->log, 0, "servername: %s", servername);

ngx_log_error(NGX_LOG_INFO, c->log, 0,
                             "44");
//   ngx_log_error(NGX_LOG_INFO, c->log, 0,
//                         "ACCEPT_CH: %s...%s (default_server)", (char *)cscf->accept_ch+10, (char *)cscf->accept_ch+50);
//   ngx_log_error(NGX_LOG_INFO, c->log, 0,
//                         "ACCEPT_CH: %s...%s", (char *)accept_ch_data+10, (char *)cscf->accept_ch+50);

//    ngx_log_error(NGX_LOG_INFO, c->log, 0,
//                          "host ACCEPT_CH: %s",&host);


//    if (!SSL_add_application_settings(ssl_conn, proto, 2, cscf->accept_ch, cscf->accept_ch_size))
   if (!SSL_add_application_settings(ssl_conn, proto, 2, accept_ch_data, accept_ch_size))
   {
//        return SSL_TLSEXT_ERR_ALERT_FATAL;
    ...
    hc = c->data;
    //ngx_str_t aa = ngx_string(servername);
// TODO: check if same req here is of same vserver; possibly try trigger re-handshake using new sni
//    if (ngx_http_find_virtual_server(c, hc->addr_conf->virtual_names, &aa,
//                                     NULL, &cscf)
//        != NGX_OK)
//    {
//        *ad = SSL_AD_NO_RENEGOTIATION;
