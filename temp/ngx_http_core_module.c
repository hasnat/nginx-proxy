...
ngx_http_core_run_phases(ngx_http_request_t *r)
{
    ...
    if (1 == 0 && r->connection->ssl) {
        ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                                  "SSL renegotiation check");
...
//        servername = ngx_pcalloc(r->pool, sizeof(ngx_str_t));

        if (servername == NULL) {
//            return NGX_ERROR;
        }

//        c = ngx_ssl_get_connection(X509_STORE_CTX_get_ex_data(x509_store, SSL_get_ex_data_X509_STORE_CTX_idx()));
...
if (1 == 0) {
        ...
        if (cscf->server_name.len && ngx_strlen(servername) &&
            ngx_strcmp(cscf->server_name.data, servername) != 0)
        {
            ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                          "SSL renegotiation required due to server name mismatch: "
                          "expected \"%s\", got \"%s\"",
                          &cscf->server_name.data, servername);

            /* Check if a renegotiation is needed */
//            if (ngx_ssl_check_need_renegotiation(r->connection) == NGX_OK) {
//                ngx_ssl_renegotiate(r->connection);
//            }
        }
        }
    }
